// Advanced skill matching algorithm without external dependencies
...
async function calculateMatchScore(job, talent) {
  try {
    ...
    // Multi-factor comprehensive matching algorithm:
    ...
    console.log(`   🎯 Skills: ${(skillFinalScore * 100).toFixed(1)}% | Exp: ${(experienceScore * 100).toFixed(1)}% | Text: ${(textSimilarityScore * 100).toFixed(1)}% | Title: ${(titleRelevance * 100).toFixed(1)}% | Loc: ${(locationScore * 100).toFixed(1)}% | Final: ${(finalScore * 100).toFixed(1)}%`);
